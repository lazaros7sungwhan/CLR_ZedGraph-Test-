...
#pragma region Delegates
		...
		{
			textBox1->Text = var2.ToString();//Data
		}
...
private: System::Void Form1_FormClosing(System::Object^ sender, System::Windows::Forms::FormClosingEventArgs^ e) {
	if (task1 != nullptr)
	{
	   ...
	   void CreateGraph2()
	   {
		   ZedGraphControl^ zg1 = gcnew ZedGraph::ZedGraphControl(); //컨트롤 선언
		   zg1->Dock = DockStyle::Fill; // 도크 채우셈
		   zg1->GraphPane = gcnew  GraphPane(); //그래프 판 새로 설정

		   LineItem^ myBar = gcnew LineItem("Bar1");
		   myBar->AddPoint(1, 10);
		   myBar->AddPoint(2, 20);
		   myBar->Line->Fill = gcnew Fill(Color::AliceBlue, Color::White, Color::AliceBlue);
		   ...
		   zg1->Show();
		   MessageBox::Show("i'm ok");
		   
		   /*if (pictureBox1->InvokeRequired)
		   {
			   tributeForPic1^ newdel2 = gcnew tributeForPic1(this, &Form1::del_pic1_Func);
			   Control::Invoke(newdel2, zg1);
		   }
		   else
		   {
			   del_pic1_Func(zg1);
